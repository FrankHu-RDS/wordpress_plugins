(function ($) {
    ...
    var freddieGalleryTheMighty  = 1000;

    if (isIE()) {
        freddieGalleryTheMighty = 10000;
    }
    ...
    setTimeout(function () {
        ...
        if($('.freddie_gallery_the_mighty ').length !== 0){
            $('.freddie_gallery_the_mighty  .et_pb_gallery_item').each(function () {
                var itemLink = $(this).find('.et_pb_gallery_caption').text();


                if(itemLink){
                    $('<span class="item_link link_exist"><span class="icon_box" urlText="'+ itemLink +'" title=""></span></span>').appendTo($(this).find('.et_overlay'))
                }else{
                    $('<span class="item_link"><span class="icon_box" title=""></span></span>').appendTo($(this).find('.et_overlay'))
                }
            ...
            $('.freddie_gallery_the_mighty .et_pb_gallery_item .item_link.link_exist .icon_box').on('click', function () {
                var thisItem = $(this);
                setTimeout(function (){
                    $('.mfp-gallery').remove();
                    $('.mfp-fade').remove();
