(function ($) {
    ...
    setTimeout(function () {
        if($('.tina_portfolio_ribbon').length !== 0){
            function masonryPortfolio() {
                var $container = $('.tina_portfolio_ribbon .et_pb_portfolio_items');
                $container.masonry({
                    columnWidth: '.project',
            ...
            if($('body.et-fb .tina_portfolio_ribbon').length !== 0){
            ...
            var hoverCount3 = 1;
            $('body.et-fb .tina_portfolio_ribbon').hover(function () {
                if (hoverCount3 === 1) {
                    $('<div class="gutter_width"></div>').appendTo('body.et-fb .tina_portfolio_ribbon .et_pb_portfolio_items');
                    masonryPortfolio();
                    hoverCount3 = 0;
                }
            ...
            if ($('body').hasClass('et-fb')) {
                setInterval(function () {
                    if (!$('body.et-fb .tina_portfolio_ribbon .et_pb_column').hasClass('divi_added')) {
                        $('<div class="gutter_width"></div>').appendTo('body.et-fb .pegasus_portfolio_3 .et_pb_portfolio_items');
                        masonryPortfolio();
                        $('body.et-fb .tina_portfolio_ribbon .et_pb_column').addClass('divi_added')
                    }
                }, 200);
            }

            $('<div class="gutter_width"></div>').appendTo('.tina_portfolio_ribbon .et_pb_portfolio_items');
            ...
            $('.tina_portfolio_ribbon .et_pb_portfolio .type-project').each(function () {
            ...
            var wrapWidhtHalf = $('.tina_portfolio_ribbon .et_pb_portfolio_items').width()/2;
            var wrapOffsetLeft = $('.tina_portfolio_ribbon .et_pb_portfolio_items').offset().left;


            $('.tina_portfolio_ribbon .et_pb_portfolio .type-project').hover(function () {
            ...
            $('.tina_portfolio_ribbon .et_pb_portfolio_items_wrapper').css('min-height', $('.tina_portfolio_ribbon .et_pb_portfolio_items').height());

            $('.tina_portfolio_ribbon .et_pb_portfolio_filters .et_pb_portfolio_filter a').on('click', function () {
                setTimeout(function () {
                    $('<div class="gutter_width"></div>').appendTo('.tina_portfolio_ribbon .et_pb_portfolio_items');
                    $('.tina_portfolio_ribbon .et_pb_portfolio_items').masonry('destroy');
                    masonryPortfolio();


                    if($('.tina_portfolio_ribbon .shadow').length === 0){
                        $('.tina_portfolio_ribbon .et_pb_portfolio .type-project').each(function () {
                            $('<div class="image_cont"></div>').appendTo($(this).find('.et_portfolio_image'))
                            $('<div class="shadow shadow_1"></div><div class="shadow shadow_2"></div><div class="shadow shadow_3"></div><div class="shadow shadow_4"></div>').appendTo($(this).find('.et_portfolio_image .image_cont'))
                            $(this).find('.et_portfolio_image img').appendTo($(this).find('.image_cont'))
            ...
            function hoverFunction(e) {
                ...
                if((itemLeft - wrapOffsetLeft) < wrapWidhtHalf-20){
                ...
                }else if((itemLeft - wrapOffsetLeft) > wrapWidhtHalf+20){
                    e.find('.shadow').each(function () {
                        transformSize2 = transformSize2 + transformSize1;
                        $(this).css('transform', 'translate(-' + transformSize2 +'px, 0)');
                    })
