...
class DIPL_MasonryGallery extends ET_Builder_Module {
		...
		return array(
			'fonts' => array(
				'title' => array(
					...
					'css'            => array(
						'main'       => "{$this->main_css_element}_lightbox .dipl_masonry_gallery_item_title",
						'text_align' => "{$this->main_css_element}_lightbox .mfp-title",
					),
					'tab_slug'	=> 'advanced',
				...
				'caption'    => array(
					...
					'css'            => array(
						'main'  => "{$this->main_css_element}_lightbox .dipl_masonry_gallery_item_caption",
					),
					'tab_slug'	=> 'advanced',
                    'toggle_slug' => 'image_text',
	...
	public function render( $attrs, $content = null, $render_slug ) {
		...
		foreach( $image_ids as $image_id ) {
			...
			if ( '' !== $title || '' !== $caption ) {
				$lightbox_content = sprintf(
					'<div class="dipl_masonry_gallery_item_lightbox_content">%1$s%2$s</div>',
			...
			if ( 'on' === $enable_lightbox ) {
				$gallery_items .= sprintf(
					...
					et_core_intentionally_unescaped( $lightbox_content, 'html' ),
					'' !== $overlay_icon ? $overlay_output : '',
					esc_url( wp_get_attachment_url( intval( $image_id ) ) )
				);
			} else {
				$gallery_items .= sprintf(
					'<div class="dipl_masonry_gallery_item">%1$s%2$s</div>',
					et_core_intentionally_unescaped( wp_get_attachment_image( intval( $image_id ), sanitize_text_field( $image_size ) ), 'html' ),
