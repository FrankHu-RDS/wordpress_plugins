...
class DIPL_ContentToggle extends ET_Builder_Module {
	...
	public function render( $attrs, $content = null, $render_slug ) {
		...
		$content_toogle_title_header_level 	=  esc_attr( $this->props['content_toogle_header_level'] ) ? esc_attr( $this->props['content_toogle_header_level'] ) : 'h5';
		$content_one 						= '';
		$content_two 						= '';

		if( '' !== $content_one_title ) {
			$content_one_title = sprintf( '<div class="dipl_toggle_title_value dipl_toggle_off_value"><%2$s>%1$s</%2$s></div>', $content_one_title, $content_toogle_title_header_level );
		}

		if( '' !== $content_two_title ) {
			$content_two_title =  sprintf( '<div class="dipl_toggle_title_value dipl_toggle_on_value"><%2$s>%1$s</%2$s></div>', $content_two_title, $content_toogle_title_header_level );
		}

		$toggle_layout = sprintf('<div class="dipl_toggle_button_wrapper layout1 dipl_toggle_%3$s">
									%1$s
			                        ...
			                        %2$s
			                   	</div>', $content_one_title, $content_two_title, $toggle_alignment );

		if( 'dipl_content_one_text' === $select_content_one_type && '' !== $content_one_text ) {
			$content_one = sprintf( '<div class="dipl_content_one_toggle dipl_content_toggle_text">%1$s</div>', $content_one_text );
		}

		if( 'dipl_content_one_layout' === $select_content_one_type && '' !== $select_content_one_layout && -1 !== $select_content_one_layout ) {
			$content_one = sprintf( '<div class="dipl_content_one_toggle dipl_content_toggle_layout">%1$s</div>', do_shortcode( get_the_content( null, false, $select_content_one_layout ) ) );
		}

		if( 'dipl_content_two_text' === $select_content_two_type && '' !== $content_two_text ) {
			$content_two = sprintf( '<div class="dipl_content_two_toggle dipl_content_toggle_text">%1$s</div>', $content_two_text );
		}

		if( 'dipl_content_two_layout' === $select_content_two_type && '' !== $select_content_two_layout && -1 !== $select_content_two_layout ) {
			$content_two = sprintf( '<div class="dipl_content_two_toggle dipl_content_toggle_layout">%1$s</div>', do_shortcode( get_the_content( null, false, $select_content_two_layout ) ) );
		}

		return sprintf('<div class="dipl_content_toggle_wrapper">%3$s%1$s%2$s</div>', $content_one, $content_two, $toggle_layout );
	}
