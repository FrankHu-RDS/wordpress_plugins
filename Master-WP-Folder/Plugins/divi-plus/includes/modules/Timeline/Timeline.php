...
class DIPL_Timeline extends ET_Builder_Module {
		...
		return array(
			...
			'timeline_padding' => array(
				'timeline' => array(
					...
					'margin_padding' => array(
						'css' => array(
							'main'      => '%%order_class%% .dipl_item_content',
							'important' => 'all',
						),
					),
					'tab_slug'       => 'advanced',
	...
	public function render( $attrs, $content = null, $render_slug ) {
		...
		$global_item_image_size                    = esc_attr( $this->props['global_item_image_size'] );
		...
		$options = array( 'dipl_item_content' => 'global_timeline_background' );
		...
		if ( '' !== $global_item_image_size ) {
			ET_Builder_Element::set_style(
				$render_slug,
				array(
					'selector'    => '%%order_class%% .dipl_image_wrapper img',
					'declaration' => sprintf( 'width: %1$s;', esc_html( $global_item_image_size ) ),
				)
			);
		}
		...
		return sprintf(
			'<div class="dipl_timeline_wrapper %2$s">
                            %1$s
                        </div><div class="%2$s_stem"><div class="dipl_stem_wrapper"><div class="dipl_stem"></div></div></div>',
			$this->content,
