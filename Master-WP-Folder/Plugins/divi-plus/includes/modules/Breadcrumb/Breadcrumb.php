...
class DIPL_Breadcrumb extends ET_Builder_Module {
	...
	public function render( $attrs, $content = null, $render_slug ) {
		...
		$nav_bg_color_hover                    = esc_attr( $this->get_hover_value( 'nav_bg_color' ) );
		...
		$nav_last_item_bg_color_hover          = esc_attr( $this->get_hover_value( 'nav_last_item_bg_color' ) );
		...
		$separator_size                        = esc_attr( $this->props['separator_size'] );
		...
		$home_link_icon_size                   = esc_attr( $this->props['home_link_icon_size'] );
		$breadcrumb_list                       = '';
		$output                                = '';
		$opacity                               = 1;
		$opacity_style                         = '';
		...
		if ( '' !== $home_link_icon_size ) {
			ET_Builder_Element::set_style(
				$render_slug,
				array(
					'selector'    => '%%order_class%% .dipl-breadcrumb-wrapper li .breadcrumb-home-icon',
					'declaration' => sprintf( 'font-size: %1$s;', esc_html( $home_link_icon_size ) ),
				)
			);
		}
		...
		if ( 'layout1' === $breadcrumb_layout && '' !== $breadcrumb ) {
			...
			if ( isset( $nav_last_item_bg_color_hover ) && 'on' === $enable_different_bg ) {
				ET_Builder_Element::set_style(
					$render_slug,
					array(
						'selector'    => '%%order_class%% .dipl-breadcrumb-wrapper.layout1 li .breadcrumb-page.dipl-last-page:hover',
						'declaration' => sprintf( 'background-color: %1$s;', esc_html( $nav_last_item_bg_color_hover ) ),
					)
				);
				ET_Builder_Element::set_style(
					$render_slug,
					array(
						'selector'    => '%%order_class%% .layout1 li .breadcrumb-page.dipl-last-page:hover:after',
						'declaration' => sprintf( 'border-color: transparent transparent transparent %1$s;', esc_html( $nav_last_item_bg_color_hover ) ),
					)
				);
				ET_Builder_Element::set_style(
					$render_slug,
					array(
						'selector'    => '%%order_class%% .layout1 li .breadcrumb-page.dipl-last-page:hover::before',
						'declaration' => sprintf( 'border-color: %1$s %1$s %1$s transparent', esc_html( $nav_last_item_bg_color_hover ) ),
					)
				);
			}
			...
			for ( $i = 0; $i < $breadcrumb_size; $i++ ) {
				...
				if ( 0 === $i ) {
					...
					if ( 'on' === $use_home_link_icon && '' !== $home_link_icon && 'off' === $hide_home_text ) {
						$breadcrumb_list .= '<li property="itemListElement" typeof="ListItem" ' . $opacity_style . '>
												    <a class="breadcrumb-item dipl-home-page" href="' . $link . '" property="item" typeof="WebPage">
												      	<span class="breadcrumb-page" property="name"><span class="breadcrumb-home-icon et-pb-icon">' . esc_attr( et_pb_process_font_icon( $home_link_icon ) ) . '</span>' . $title . '</span>
												    </a>
						...
						$breadcrumb_list .= '<li property="itemListElement" typeof="ListItem" ' . $opacity_style . '>
								    <a class="breadcrumb-item dipl-home-page" href="' . $link . '" property="item" typeof="WebPage">
								      	<span class="breadcrumb-page" property="name">' . $title . '</span>
								    </a>
								    <meta property="position" content="' . ( $i + 1 ) . '" />
								</li>';
					}
					...
					$breadcrumb_list .= '<li property="itemListElement" typeof="ListItem" ' . $opacity_style . '>
								      		<span class="breadcrumb-page dipl-last-page" property="name">' . $title . '</span>
								    		<meta property="position" content="' . ( $i + 1 ) . '" />
								  		</li>';
				} else {
					$breadcrumb_list .= '<li property="itemListElement" typeof="ListItem" ' . $opacity_style . '>
											    <a class="breadcrumb-item" href="' . $link . '" property="item" typeof="WebPage">
											      <span class="breadcrumb-page" property="name">' . $title . '</span>
											    </a>
											    <meta property="position" content="' . ( $i + 1 ) . '" />
											</li>';
		...
		if ( 'layout2' === $breadcrumb_layout && '' !== $breadcrumb ) {
			...
			if ( 'text_separator' === $separator_type ) {
				$separator_content = ' <span class="dipl-breadcrumb-separator">'
										. $separator_text .
									'</span> ';
			} else {
				$separator_content = ' <span class="dipl-breadcrumb-separator et-pb-icon">'
			...
			if ( '' !== $separator_size ) {
				ET_Builder_Element::set_style(
					$render_slug,
					array(
						'selector'    => '%%order_class%% .dipl-breadcrumb-wrapper .dipl-breadcrumb-separator',
						'declaration' => sprintf( 'font-size: %1$s;', esc_html( $separator_size ) ),
					)
				);
			}
			...
			if ( isset( $separator_color_hover ) ) {
				ET_Builder_Element::set_style(
					$render_slug,
					array(
						'selector'    => '%%order_class%% .dipl-breadcrumb-wrapper .dipl-breadcrumb-separator:hover',
			...
			for ( $i = 0; $i < $breadcrumb_size; $i++ ) {
				...
				if ( 0 === $i ) {
					$title = ( '' !== $title ) ? $title : $home_link_text;
					if ( 'on' === $use_home_link_icon && '' !== $home_link_icon && 'off' === $hide_home_text ) {
						$breadcrumb_list .= '<li property="itemListElement" typeof="ListItem">
												    <a class="breadcrumb-item dipl-home-page" href="' . $link . '" property="item" typeof="WebPage">
												      	<span class="breadcrumb-page" property="name"><span class="breadcrumb-home-icon et-pb-icon">' . esc_attr( et_pb_process_font_icon( $home_link_icon ) ) . '</span>' . $title . '</span>
												    </a>
												    <meta property="position" content="' . ( $i + 1 ) . '" />
												</li>' . $separator_content;
