...
$wps_ic_active_settings['url_replace']['cdn']          = '';
$wps_ic_active_settings['url_replace']['direct']       = '';

/**
 * Decides which setting is active
 */

if ( ! empty($settings['url-method'])) {
	if ($settings['url-method'] == 'cdn') {
		$wps_ic_active_settings['url_replace']['cdn'] = 'class="current"';
	} else if ($settings['url-method'] == 'direct') {
		$wps_ic_active_settings['url_replace']['direct'] = 'class="current"';
	}
} else {
	$wps_ic_active_settings['url_replace']['cdn'] = 'class="current"';
}
...
if ($quota_type == 'shared') {
	...
	if ($user_credits->bytes->local_leftover > 0) {
		$local_credits = number_format($user_credits->bytes->local_leftover, 0) . ' Local Images Left';
	} else {
		$local_credits = '0 Local Images Left';
	}

	// Quota
	if ($user_credits->bytes->leftover > 0) {
		$credits = number_format($user_credits->bytes->leftover, 0) . ' Live Requests Left';
	} else {
		$credits = '0 Live Requests Left';
	}
...
/**
 * Purge CDN - fallback from ajax fail
 */
if ( ! empty($_GET['purge_cdn'])) {
	$cnd_Purged = 'false';
	$options    = get_option(WPS_IC_OPTIONS);
	$url        = 'http://keys-live.wpmediacompress.com/?action=purgeCDN&apikey=' . $options['api_key'];
      ...
      <div class="wp-compress-header">
        ...
        <div class="wp-ic-header-buttons-container">
          <ul>
            ...
            <li class="bulk-button" style="<?php if ( ! empty($live_cdn) && $live_cdn == '1') {echo 'display:none;';} ?>"><a href="<?php echo admin_url('options-general.php?page=wpcompress&view=bulk'); ?>" class="button button-primary">Bulk Optimization</a></li>
                      ...
                      <div class="image-credits-remaining">
                        ...
                        <a href="https://wpcompress.com/pricing" target="_blank" class="button button-primary requests-left" style="<?php echo $requests_left; ?>">
                          <h5><?php echo $credits; ?></h5>
												...
												<?php
												if ($quota_type == 'requests' || $quota_type == 'shared_requests') {
													#echo '<div class="free-badge">+1000 Bonus</div>';
												} else {
													echo '<div class="free-badge">+1 GB Bonus</div>';
												}
												?>
                      </div>
                    </div>
