...
class wps_local_compress {
	...
	public function __construct() {
		...
		#self::$apiURL = 'https://singapore-01.wpmediacompress.com/key:' . self::$options['api_key'] . '/q:' . self::$settings['optimization'] . '/retina:false/webp:false/w:1/url:';
		self::$apiURL = 'https://' . self::$zone_name . '/local:true/q:' . self::$settings['optimization'] . '/retina:false/webp:false/w:1/url:';
		#self::$apiURL = 'https://dev.wpmediacompress.com/key:' . self::$options['api_key'] . '/q:' . self::$settings['optimization'] . '/retina:false/webp:false/w:1/url:';
	}
	...
	public function compress_image($imageID, $retina = true, $webp = true) {
		...
		foreach ($this->sizes as $i => $size) {
			...
			$call = wp_remote_get($apiURL, array('timeout' => 10, 'sslverify' => false, 'user-agent' => 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.8; rv:20.0) Gecko/20100101 Firefox/20.0'));
	...
	public function generate_retina($arg) {
		...
		foreach ($this->sizes as $i => $size) {
			...
			$call = wp_remote_get($retinaAPIUrl, array('timeout' => 10, 'sslverify' => false, 'user-agent' => 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.8; rv:20.0) Gecko/20100101 Firefox/20.0'));
			if (wp_remote_retrieve_response_code($call) == 200) {
				$body = wp_remote_retrieve_body($call);
				if ( ! empty($body)) {
	...
	public function generate_webp($arg) {
		...
		foreach ($this->sizes as $i => $size) {
			...
			$call = wp_remote_get($webpAPIUrl, array('timeout' => 10, 'sslverify' => false, 'user-agent' => 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.8; rv:20.0) Gecko/20100101 Firefox/20.0'));
			if (wp_remote_retrieve_response_code($call) == 200) {
				$body = wp_remote_retrieve_body($call);
				if ( ! empty($body)) {
